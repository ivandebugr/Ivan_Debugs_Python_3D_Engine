...
## Plane-projection for cursor-pinned gizmo drag (ray-line closest point) — 2026-07-07
**Problem:** A velocity-based translate-gizmo drag (`mouse_vel.dot(axis_screen) * SCALE` added per frame) is speed-dependent, not position-dependent — a fast flick moves the entity further than a slow drag across the identical screen distance. This reads as "imprecise" because the handle never actually tracks the cursor.
**Solution:** On grab, build a drag plane containing the axis and oriented to face the camera (`normal = axis × (axis × view_dir)`), and record the axis-space offset between the grabbed point and the gizmo origin. Each frame: intersect the cursor ray with that fixed plane (`_ray_plane_hit`), project the hit onto the infinite axis line (`_closest_point_on_line`), and move the entity by the delta between the new offset and the grabbed offset — not by an incremental per-frame velocity. This makes drag distance a pure function of cursor *position*, so identical start/end cursor positions produce identical entity displacement regardless of how many frames or how fast the mouse moved between them (verified: single big jump == many small steps to the same endpoint). Guard the near-parallel-to-view edge case (screen-space axis length below a threshold) by falling back to the old velocity model — plane-projection is numerically unstable there since the ray barely intersects the plane at a useful point.
**Used in:** `Scripts/editor_gizmo.py` (`GizmoController._begin_drag`/`handle_drag`/`_ray_plane_hit`/`_closest_point_on_line`/`_drag_plane_normal`)
**Source:** [[work/active/v1.7-editor-ux-scoping]] B1; [[work/archive/2026/v1.7-editor-ux-bundle]]

## Effective-width property pair for collapsible camera.ui panels — 2026-07-07
**Problem:** `camera.ui` panel width/position is baked into several files' layout math as a raw class constant (`_LAYOUT_HIER_W`, `_LAYOUT_INSP_W`) — the hierarchy panel, inspector panel, and asset browser (tab row, card grid, scroll indicators) all read those constants directly to avoid overlapping each other. Making a panel collapsible means every one of those read sites needs to know the *current* width, not the constant, or the "reclaimed" space isn't actually reclaimed and neighboring UI doesn't re-center.
**Solution:** Keep the constant (still the expanded-state width) but add an `_effective_*_w` property on the owner that returns the constant when the panel is visible and a thin fixed strip (just wide enough for a collapse-toggle chevron) when it isn't. Every layout call site that referenced the raw constant switches to the property instead — one property, many read sites, single toggle in `panel_visible` drives them all consistently. Pair this with an `.enabled`-gated `_is_over_panel`/`_is_over_browser` check so a collapsed (but still nonzero-width) panel doesn't keep intercepting clicks meant for the viewport underneath it.
**Used in:** `Scripts/editor_core.py` (`_effective_hier_w`/`_effective_insp_w`/`_effective_browser_h`, `_toggle_panel`), `Scripts/editor_hierarchy.py`/`editor_inspector.py`/`editor_browser.py` (`apply_layout` call sites)
**Source:** [[work/active/v1.7-editor-ux-scoping]] C1; [[work/archive/2026/v1.7-editor-ux-bundle]]
