...
| v1.7 | Shipped 2026-07-18 | — | Lighting pass (L1 + L3), bloom post-process, particle effects; viewmodel offscreen-buffer fix for point-blank wall clipping (dual-camera → dedicated `FrameBufferProperties` buffer); shotgun pellet-spread + point-blank hit fixes; muzzle-flash fixes; level editor UX fixes across inspector/layout/color/dedup. See [[brain/Gotchas]] for the shader/depth-clear investigations from this cycle |
