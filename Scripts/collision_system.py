...
class Layers:
    ...
    ENEMY_BULLET  = 1 << 3   # 8
    WALL          = 1 << 4   # 16
    PICKUP        = 1 << 5   # 32 — AmmoPickup (v1.5 Step 12). NOT a damage path; never
                             #      appears in COLLISION_MATRIX as attacker. Overlap is
                             #      per-frame self.intersects(), same pattern as TRIGGER.
...
def swept_move_blocked(mover, origin, direction, dist, offsets, skin_width=0.0) -> bool:
    ...
    bullet mid-air), trigger AND pickup volumes are always ignored (Layers.TRIGGER
    and Layers.PICKUP are enter/exit detection volumes, never physical blockers —
    v1.5), and `mover` ignores itself. Walls are unregistered, so they are NOT in
    the ignore lists and correctly block the sweep.
