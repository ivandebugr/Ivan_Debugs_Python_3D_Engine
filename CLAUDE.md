...
15. `Scripts/audio_workaround.py` must be imported before `from ursina import *` in every entry point — ursina's import-time `create_AudioManager()` otherwise crashes libp3openal on this Mac (SIGSEGV in OpenAL Soft's HRTF loader). The module disables HRTF via `ALSOFT_CONF` and selects real OpenAL (`p3openal_audio`) so audio works on the pinned 1.10.16; do not revert it to `audio-library-name null`. See `brain/Gotchas.md`.
